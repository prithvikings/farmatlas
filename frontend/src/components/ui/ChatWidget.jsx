...
export default function ChatWidget() {
  ...
  const handleSendMessage = (e) => {
    ...
    // 2. Simulate Network Delay (Thinking)
    setTimeout(() => {
      const randomResponse = MOCK_ANSWERS[Math.floor(Math.random() * MOCK_ANSWERS.length)]
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md border-b border-zinc-100 dark:border-zinc-800 absolute top-0 w-full z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-semibold text-sm text-zinc-900 dark:text-zinc-100 leading-tight">Livestock AI</div>
              <div className="text-[11px] text-zinc-500 font-medium">Support Agent</div>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 px-5 pt-24 pb-4 overflow-y-auto no-scrollbar bg-zinc-50 dark:bg-zinc-950/50">
          <div className="flex flex-col gap-4">
             <div className="text-center">
                <span className="text-[10px] font-medium text-zinc-400 bg-zinc-100 dark:bg-zinc-800/50 px-2 py-1 rounded-full">Today</span>
             </div>

            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex w-full animate-message-in flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}
              >
                <div
                  className={`
            ...
            {isTyping && (
              <div className="flex flex-col items-start animate-message-in">
                <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-4 py-3 rounded-2xl rounded-tl-none flex gap-1 items-center shadow-sm w-fit">
                  <div className="w-1.5 h-1.5 bg-zinc-400 rounded-full animate-typing-dot" style={{ animationDelay: '0s' }}></div>
                  <div className="w-1.5 h-1.5 bg-zinc-400 rounded-full animate-typing-dot" style={{ animationDelay: '0.2s' }}></div>
                  <div className="w-1.5 h-1.5 bg-zinc-400 rounded-full animate-typing-dot" style={{ animationDelay: '0.4s' }}></div>
                </div>
              </div>
            )}
        ...
        {/* Input Area */}
        <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800">
