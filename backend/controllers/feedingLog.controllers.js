...
export const createFeedingLog = async (req, res) => {
  try {
    ...
    if (animal.farmId.toString() !== farmId) {
      return res.status(403).json({ message: "Cannot create feeding log for another farm." });
    }
    ...
    return res.status(500).json({ message: "Error creating feeding log.", error: error.message });
  }
};
...
export const getFeedingLogsByAnimal = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "Error fetching feeding logs.", error: error.message });
  }
};
...
export const updateFeedingLog = async (req, res) => {
  try {
    ...
    const updated = await FeedingLog.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true }
    );
    ...
    return res.status(500).json({ message: "Error updating feeding log.", error: error.message });
  }
};
...
export const deleteFeedingLog = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Feeding log deleted successfully" });

  } catch (error) {
    console.error("deleteFeedingLog error:", error);
    return res.status(500).json({ message: "Error deleting feeding log.", error: error.message });
  }
};
